...
install.packages("./ipayipi_0.0.7.tar.gz")
...
library(ipayipi)
# set package options for functions not to run in parallel
#  good option for testing function implementation
...
pd <- "ipayipi_vignettes/met_cs_qa_eg" # relative working directory

# run ipip_house to generate the folder structure for the pipeline functioning
pipe_house <- ipip_house(pd)
...
logger_data_import_batch(pipe_house, wanted = "vasi")

# reads data into R format
#  - sorts out time intervals + organise header, phen data
imbibe_raw_batch(pipe_house, data_setup = ipayipi::cs_stom,
  verbose = TRUE
)
...
phenomena_sts(pipe_house, external_phentab = ipayipi::sts_phens,
  verbose = TRUE
)
...
source(
  "ipayipi_vignettes/met_cs_qa_eg/r/pipe_seq/pipe_seq_cs_aws_vasi_sci_v05_l1.r"
)
...
dt_process_batch(pipe_house = pipe_house, pipe_seq = pipe_seq,
  overwrite_pipe_memory = TRUE, verbose = TRUE, xtra_v = TRUE
)
...
dta_flat_pull(pipe_house = pipe_house,
  tbl_names = "dt_1_days_agg", # vector of table names
...
# generate a flag summary
dt <- fread(file.path(
  "ipayipi_vignettes/met_cs_qa_eg/dta_out",
  "mcp_vasi_science_aws-dt_flag_raw_5_mins.csv"
))
dt[, names(dt)[names(dt) %ilike% "^flag"] := lapply(
  .SD, function(x) sum(x, na.rm = TRUE)
), .SDcols = names(dt)[names(dt) %ilike% "^flag"]]
dt <- unique(dt[, names(dt)[names(dt) %ilike% "^flag"], with = FALSE])
fwrite(dt, file.path(pipe_house$dta_out,
  "mcp_vasi_sci_centre_aws-dt_flag_raw_5_mins_summary.csv"
))