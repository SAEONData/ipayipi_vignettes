...
The overview below shows code needed to execute the steps above. Each of these steps are described further in this vignette. For more details and options for each step, see function documentation/help files in R (e.g., {?ipip_house()}). These help files serve as documentation protocol for each section of the pipeline.
...
## define pipeline working directory 'pd' (must be reletive)
...
2. 'wait_room': waiting room for imbibing & standardising data coming into the pipeline.
...
*NB! _Running this function will not overwrite existing data_. Also, it will not create the base directory ('pipe_house_dir') for you --- this must exist before running the function.

# Imbibe data

In this step, incoming data gets pulled from pipeline's data source, that is, the 'source room' (`pipe_house$source_room`), into the 'waiting room' (`pipe_house$wait_room`). The example data contains multiple years of hobo-rainfall data exports from [SAEON](https://www.saeon.ac.za) rainfall stations in northern Maputaland, South Africa.
...
```
For additional insight into data-input formats, that is, the 'data_setup' argument, _see_ the help files of the `imbibe_raw_logger_dt()` function (i.e., `?imbibe_raw_logger_dt`). The `ipayipi::ipayipi::hobo_rain` caters for five variations in data formats associated with Hoboware exports (plus many variations of date-time stamp formats within each of these). These import format variations allow the import of hundreds of differently formatted (unencrypted) Hobo data file exports---you can add your own custom versions too.
...
If it is the first-time running `header_sts()`, or new synonyms get introduced into pipe-house directory, `header_sts()` will produce a message. This normally happens when a station name or logger program get changed, or with the introduction of a new station into the pipeline---for this, the pipeline has to update its nomenclature standards.

<span style="background-color:#df9a86"> Unstandardised names (or columns) have the preffix '**uz**'</span>. These standards get stored in a file called 'aa_nomtab.rns' in the 'waiting room'. _If this file is deleted---a new one will be generated---but the user will have to populate the tables with synonym vocab_.

The nomenclature table in the 'waiting room' can be updated from the csv file (or directly in R). If a new synonym gets introduced---the file containing new nomenclature will be skipped in further processing---a 'csv' version of the 'aa_nomtab.rns' will be copied to the 'waiting room' for editing.
...
- <span style="background-color:#aed8f0">standard title</span>: station name. A concatenation of location and station with suffix denoting the type of station; for rain gauges this suffix is '_pr'. 
...
\* Note the standard table name for the 'raw', precipitation-event data, 'raw_rain' --- keeping this name standard is important in further processing.
...
Standardising _phenomena_ metadata follows a similar process as for header-data standardisation. If the phenomena standards have been described and there is a 'aa_phentab.rps' in the 'waiting room', running the below code updates all files phenomena details.
...
**Tip**: Setting the `remove_dups` argument to `TRUE` in `phenomena_sts()` allows the user to interactively remove, through a terminal prompt, duplicate phenomena, on the off chance logger exports have included these. _Any interactive (via the prompt line) functionality is only possibly if parallel processing is sequential. See the section on parallel processing near at the end of the vignette_.


If there is no 'phenomena table' ('aa_phentab.rps'), one `NA` values in the 'csv' copy need to be described. The following fields in the 'csv' phentab must be populated:
...
Standardised data files get transferred to the 'nomenclature vetted' directory (i.e., the 'nomtab room') using the function below. After being transferred, files in the waiting room (except the nomtab and phentab standards) are automatically removed.

Note in the 'waiting room' how the file extension of files change as they are standardised. Successfully **imbibed** files have a '**ipr**' extension. Post **header** standardisation the extension changes to '**iph**'; the extension becomes '**ipi**' when **phenomena** synonyms are corrected. In summary the change is from * 'ipr' -> 'iph' -> 'ipi'.
...
__Handling big data__: In order to minimise memory usage when processing station files, especially optimised for adding segments of new data, these are 'decompressed' into chunks in an R session. The temporary files are indexed/chunked/read by date-time (if present). While this reduces memory allocation for further processing there is an initial time cost to chunk data when starting to use a station in an R session using this default setting. This initial cost can be reduced for advanced users, more of this near the document end.
...
meta_read(meta_file = rl, col_types = "ilTccccdddcllcTTc",
  output_dir = pipe_house$ipip_room, output_name = "aa_event_db"
)
```
...
p <- dta_availability(pipe_house = pipe_house, tbl_names = "raw_rain")
...
# Process data
Next data gets processed using a custom 'pipe sequence'. The 'pipe sequence' documents processing operations for consecutive stages, each with nested steps. The pipe sequence object `pipe_seq` is read into the R environment by sourcing its script as below. This script serves as a reference for processing data. More on the pipe sequence at the end of this document.

```{r embed pipe_sequence, eval = TRUE}
source('ipayipi_vignettes/rainfall_eg/r/pipe_seq/txs_tb.r')
dt_process_batch(pipe_house = pipe_house, pipe_seq = pipe_seq, overwrite_pipe_memory = TRUE)
```
...
d <- dta_flat_pull(pipe_house = pipe_house, phen_name = "rain_tot",
  tbl_names = "dt_1_years_agg", wide = FALSE
)
d$dta
```
...
#' aggregation
x22 <- list(
  fork_se1 = chainer(j = "fktest := fifelse(.I == 1 | .I == .N, TRUE, FALSE)"),
  fork_se2 = chainer(i = "fktest == TRUE"),
  fork_se3 = chainer(j = "rain_mm := rain_cumm",
...
#' finalise false tips
x47 <- list(
  false_tip_type1 = chainer(j = c("false_tip_type := fifelse(",
...
# Tips for working in 'ipayipi'
 ...
 - It makes sense to start with on station or fewer files to learn how the process works before processing too much data.
...
# Appendix I: Session information
