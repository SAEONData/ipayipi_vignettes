...
pd <- "ipayipi_vignettes/rainfall_eg"
...
meta_to_station(pipe_house = pipe_house, meta_file = "aa_event_db",
  verbose = TRUE
)
...
dt_process_batch(pipe_house = pipe_house, pipe_seq = pipe_seq, verbose = TRUE,
  overwrite_pipe_memory = TRUE, xtra_v = TRUE
)
dta_flat_pull(pipe_house = pipe_house,
  phen_name = "rain_tot", # name of the phenomena --- exact match
  tbl_names = "dt_1_years_agg", # table within which to search for phen
  out_csv = TRUE,
  out_csv_preffix = "mcp"
)
